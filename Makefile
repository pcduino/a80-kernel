...
A80_CONFIG=$(SOURCE_K)/.config
all: linux-A80 livesuit

clean:
	rm -rf $(BUILD_PATH)

## patch
patch:  $(SOURCE_K)/.git u-boot-sunxi/.git
...
$(K_DOT_CONFIG):
	$(Q)mkdir -p $(K_O_PATH)
	$(Q)$(MAKE) -C $(SOURCE_K) O=$(K_O_PATH) ARCH=arm $(KERNEL_CONFIG)

linux: $(K_DOT_CONFIG)
	$(Q)$(MAKE) -C $(SOURCE_K) O=$(K_O_PATH) ARCH=arm oldconfig
	$(Q)$(MAKE) -C $(SOURCE_K) O=$(K_O_PATH) ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} -j$J INSTALL_MOD_PATH=output uImage modules
	$(Q)$(MAKE) -C $(SOURCE_K) O=$(K_O_PATH) ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} -j$J INSTALL_MOD_PATH=output modules_install
	cd $(K_O_PATH) && ${CROSS_COMPILE}objcopy -R .note.gnu.build-id -S -O binary vmlinux bImage
##a80 linux
$(A80_CONFIG):linux-a80-3.4/.git
	cd $(SOURCE_K)	&& $(MAKE)  ARCH=arm $(KERNEL_CONFIG)

linux-A80: 
	cd $(SOURCE_K) && ./scripts/build.sh
	
linux-config: $(K_DOT_CONFIG)
	$(Q)$(MAKE) -C $(SOURCE_K) O=$(K_O_PATH) ARCH=arm menuconfig
